...
export default function addForm() {
	count++;
	const formContainer = document.getElementById('member_registration');
	const li = document.createElement('li');

	li.className = 'question-item mt-20 flex justify-between';

	li.innerHTML = `
        <label class="w-70">
            <span class="question-title">お名前 ${count}</span>
            <input type="text" name="user_name" placeholder="例）ニックネーム">
        </label>
        <label class="w-30" for="role">
            <span>権限を選ぶ</span>
            <select name="parent" id="role">
                <option value="0">管理者</option>
                <option value="1" selected>メンバー</option>
            </select>
        </label>
		<button type="button" id="${count}" class="button_trash">
    ...
    `;
	formContainer.appendChild(li); // 新しいフォームを追加
}
